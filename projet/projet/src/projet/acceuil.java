...
public class acceuil extends JFrame implements ActionListener {
	...
	public acceuil(){
		...
		String lv="select titre from livre";
try{
	st=cn.getInstance().createStatement();
	rst=st.executeQuery(lv);
	while(rst.next()){
	jcb1.addItem(rst.getString("titre"));
		...
		String ab="select nomab from abonne";
try{
	st=cn.getInstance().createStatement();
	rst=st.executeQuery(ab);
	while(rst.next()){
	...
	//jcb3	
		...
		tb3.setModel(df3);
		pn.add(js3);
		String dis="select idlivre,titre from livre where disponible='OUI'";
		try{
			st=cn.getInstance().createStatement();
			rst=st.executeQuery(dis);
			while(rst.next()){
	...
	public void actionPerformed(ActionEvent e) {
		...
		try{
			...
			JOptionPane.showMessageDialog(this,"Erreur, impossible!",null,JOptionPane.ERROR_MESSAGE);
		}
